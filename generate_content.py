...
def can_run_script() -> bool:
    print("[STEP] Checking last post status in ujjawal_linkedin_topics.json...", flush=True)
    topics_file = Path("ujjawal_linkedin_topics.json")
    if not topics_file.exists():
        print("[WARNING] 'ujjawal_linkedin_topics.json' nahi mili. Proceeding by default...", flush=True)
        return True
...
def get_next_topic_from_json() -> str:
    print("[STEP] Reading ujjawal_linkedin_topics.json...", flush=True)
    topics_file = Path("ujjawal_linkedin_topics.json")
    if not topics_file.exists():
        raise FileNotFoundError("❌ 'ujjawal_linkedin_topics.json' file nahi mila.")
    
    with topics_file.open("r", encoding="utf-8") as f:
        topics = json.load(f)
    
    if not topics:
        raise ValueError("❌ 'ujjawal_linkedin_topics.json' khali hai.")
    ...
    raise ValueError("❌ 'ujjawal_linkedin_topics.json' mein koi naya unprocessed topic nahi mila.")


def update_topic_status_in_json(topic_text: str):
    print("[STEP] Updating topic status in ujjawal_linkedin_topics.json...", flush=True)
    topics_file = Path("ujjawal_linkedin_topics.json")
    if not topics_file.exists():
        return
    ...
    print("[OK] Topic status successfully updated (content_generated=True) in ujjawal_linkedin_topics.json", flush=True)
