...
def can_run_image_script() -> tuple:
    print("[STEP] Checking status in ujjawal_linkedin_topics.json...", flush=True)
    topics_file = Path("ujjawal_linkedin_topics.json")
    if not topics_file.exists():
        print("[INFO] 'ujjawal_linkedin_topics.json' nahi mili. Execution stopped.", flush=True)
        return False, None
...
def update_image_status_in_json(topic_text: str):
    print("[STEP] Updating image status in ujjawal_linkedin_topics.json...", flush=True)
    topics_file = Path("ujjawal_linkedin_topics.json")
    if not topics_file.exists():
        return
    ...
    print("[OK] Image status successfully updated (image_generated=True) in ujjawal_linkedin_topics.json", flush=True)
...
def run():
    ...
    try:
        ...
        prompt = (
            "Generate a brand-new image from scratch based ONLY on the LinkedIn post below.\n\n"

            "This is a TEXT-TO-IMAGE generation task.\n"
            "Do NOT ask for a reference image.\n"
            "Do NOT assume an existing image exists.\n"
            "Do NOT edit, modify, enhance, or transform any image.\n"
            "Create a completely original image from scratch.\n\n"

            "OBJECTIVE:\n"
            "Create a premium LinkedIn visual that instantly communicates the post's core insight, even if the viewer never reads the caption.\n"
            "The image should maximize curiosity, stop scrolling, and encourage engagement.\n\n"

            "VISUAL STYLE:\n"
            "Choose whichever of these best fits the topic:\n"
            "- Editorial photography\n"
            "- Cinematic photography\n"
            "- Minimalist concept art\n"
            "- Premium conceptual digital artwork\n\n"

            "Avoid stock-photo aesthetics, generic LinkedIn visuals, corporate handshakes, people pointing at charts, gavels, courtroom clichés, floating icons, and obvious metaphors.\n\n"
            ...
            "- High contrast\n"
            "- Realistic lighting and materials when appropriate\n\n"
            ...
            "Do NOT request a reference image.\n"
            "Do NOT say you need an uploaded image.\n"
